...
const findByIds = async <T extends new (...args: unknown[]) => unknown>(
  ...
  const connection = findConnection(type);
  const repository = getRepository<InstanceType<T>>(type, connection);
  const values = validIds.length ? await repository.findByIds(validIds) : [];
